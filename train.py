"""
train.py -- training loop with: from_scratch pretraining, jepa_enabled switch,
loss-dropout, configurable JEPA loss, LR warmup+cosine, checkpoint/resume,
edge-pass timing (for the loss-dropout speed study). Multi-GPU via Accelerate.
Returns a dict of run stats (timing) so study scripts can record speed.
"""
import os
import time
import random
...
def train():
    ...
    acc.print(f"\n{'='*55}\n  MODE: from_scratch={CFG.from_scratch} | "
              ...
              f"loss_dropout={CFG.loss_dropout}\n{'='*55}\n")
            ...
            with acc.accumulate(model):
                ...
                if CFG.jepa_enabled:
                    ...
                    for (i, j) in edges:
                        ...
                        pe = base.predict_stage(ctx); te = base.encode(tgt)
                        edges_run += 1
